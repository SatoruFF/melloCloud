      ...
      <List
        ...
        renderItem={(item, index) => (
          <List.Item className={cn(styles.todoItem)}>
            <Checkbox checked={item.completed} onChange={() => toggleTask(index)}>
              <span className={cn({ [styles.completed]: item.completed })}>{item.text}</span>
            </Checkbox>
            <Button type="text" icon={<DeleteOutlined />} onClick={() => deleteTask(index)} danger />
          </List.Item>
        )}
      />
    </div>
